(function(){
	...
	if (!tasksStore) {
		...
		tasksStore.split(',').filter(item => item.length > 0).forEach(item => {
		...
		var current_tasks = document.querySelectorAll(".delete");
			for(var i=0; i<current_tasks.length; i++){
				current_tasks[i].onclick = function(){
					...
					const arr = localStorage.getItem('tasks').split(',').filter(item => item.length > 0);
					arr.splice(index, 1);
	...
	document.querySelector('#push').onclick = function() {
		if(document.querySelector('#newtask input').value.length == 0){
			...
			const arr1 = localStorage.getItem('tasks').split(',').filter(item => item.length > 0);
